...
@pytest.fixture
def client(current_user_id,isolated_upload_dir,mock_worker_client):
    shared_repo = FakeDocumentRepository()
    app.dependency_overrides[get_document_service] = lambda: DocumentService(shared_repo, mock_worker_client)
...
@pytest.mark.asyncio
async def test_delete_document_returns_204_and_it_disappears_from_the_list(client, mock_worker_client):
    with patch("app.services.document_service.extract_text",return_value=("some text",1)), \
         patch("app.services.document_service.chunk_text",return_value=["some text"]), \
         patch("app.services.document_service.celery_app"):
        async with client as ac:
            ...
            delete_response = await ac.delete(f"/api/v1/documents/{document_id}")
            ...
            list_response = await ac.get("/api/v1/documents/")
            assert all(d["id"] != document_id for d in list_response.json())

    mock_worker_client.delete.assert_awaited_once()


@pytest.mark.asyncio
async def test_delete_unknown_document_returns_404(client, mock_worker_client):
    async with client as ac:
        response = await ac.delete(f"/api/v1/documents/{uuid4()}")
...
@pytest.mark.asyncio
async def test_user_cannot_delete_another_users_document(client, current_user_id, mock_worker_client):
    with patch("app.services.document_service.extract_text",return_value=("some text",1)), \
         patch("app.services.document_service.chunk_text",return_value=["some text"]), \
         patch("app.services.document_service.celery_app"):
        async with client as ac:
    ...
    app.dependency_overrides[get_current_user] = lambda: CurrentUser(id=uuid4(), raw_claims={})
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.delete(f"/api/v1/documents/{document_id}")
