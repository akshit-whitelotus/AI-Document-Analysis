...
@pytest.mark.asyncio
async def test_register_creates_user_with_hashed_password(fake_user_repository,valid_user_create):
...
@pytest.mark.asyncio
async def test_register_rejects_duplicate_username(fake_user_repository,valid_user_create):
    service=AuthService(fake_user_repository)
    await service.register(valid_user_create)
...
@pytest.mark.asyncio
async def test_login_succeeds_with_correct_credentials(fake_user_repository,valid_user_create):
    service=AuthService(fake_user_repository)
    await service.register(valid_user_create)
...
@pytest.mark.asyncio
async def test_login_rejects_wrong_password(fake_user_repository,valid_user_create):
    service=AuthService(fake_user_repository)
    await service.register(valid_user_create)
...
@pytest.mark.asyncio
async def test_login_rejects_unknown_email(fake_user_repository):
    service=AuthService(fake_user_repository)
    with pytest.raises(UnauthorizedException):
        await service.login(UserLogin(email="nobody@example.com",password="whatever1234"))

@pytest.mark.asyncio
async def test_login_rejects_disabled_account(fake_user_repository,valid_user_create):
    service=AuthService(fake_user_repository)
    user=await service.register(valid_user_create)
...
@pytest.mark.asyncio
async def test_refresh_issues_a_valid_access_token(fake_user_repository,valid_user_create):
    service=AuthService(fake_user_repository)
    user=await service.register(valid_user_create)
...
@pytest.mark.asyncio
async def test_refresh_rejects_an_access_token(fake_user_repository,valid_user_create):
    service = AuthService(fake_user_repository)
    await service.register(valid_user_create)
    login_tokens= await service.login(UserLogin(email=valid_user_create.email,password=valid_user_create.password))
    with pytest.raises(UnauthorizedException):
